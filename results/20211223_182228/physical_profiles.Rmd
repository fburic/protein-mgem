---
title: "Produce physical sequence profiles"
---
...
prot_seq <- tibble(
    swissprot_ac = names(prot_seq),
    seq = paste(prot_seq)
) %>% 
dplyr::mutate(swissprot_ac = base::sapply(
    swissprot_ac,
    function(id_str) {
       stringr::str_split(id_str, pattern = "\\|")[[1]][[2]]
    })
)
```
...
aa_indices <- uncorr_indices
aa_indices <- c(aa_indices, "WOEC730101")

uncorr_indices
```

```{r}
hist(aaindex_avg_corr_matrix[, "WOEC730101"])
```

```{r}
sum(aaindex_avg_corr_matrix[, "WOEC730101"] < 0.2, na.rm = T)
```

```{r}
plot(aaindex_avg_corr_matrix[c(aa_indices), "WOEC730101"])
```

```{r}
reshape2::melt(na.omit(aaindex_avg_corr_matrix[c(aa_indices),
                                               c(aa_indices)])) %>% 
ggplot(aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() +
  scale_fill_gradient2(low = "white", high = "black", na.value = "pink") +
...
```{r}
# aa_indices <- c("KYTJ820101",
#                 "ARGP820101", 
#                 "WOEC730101", 
#                 "ZIMJ680104",
#                 "KIMC930101",
#                 "KOEP990101")
prog_bar <- progress_bar$new(total = length(aa_indices))
...
```{r}
protein_ids <- prot_profiles %>% select(swissprot_ac) %>% pull
```
...
if (!h5createFile(store_file)){
    ...
    for (prot_id in protein_ids) {
        ...
        prot_data <- prot_profiles %>% dplyr::filter(swissprot_ac == prot_id)
